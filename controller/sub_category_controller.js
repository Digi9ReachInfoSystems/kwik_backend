...
exports.getAllSubCategories = async (req, res) => {
    try {
        ...
        res.status(200).json({ message: 'Sub-categories retrieved successfully', data: subCategories });
    } catch (error) {
...
exports.addSubCategory = async (req, res) => {
    try {
        const { sub_category_id, category_ref, sub_category_name, sub_category_des, sub_category_image } = req.body;

        // Validate required fields
        if (!sub_category_id || !category_ref || !sub_category_name || !sub_category_des || !sub_category_image) {
        ...
        if (!categoryExists) {
            return res.status(400).json({ message: 'Invalid category_ref. The referenced category does not exist.' });
        }
        ...
        res.status(201).json({ message: 'Sub-category added successfully', data: savedSubCategory });
    } catch (error) {
...
exports.editSubCategory = async (req, res) => {
    try {
        ...
        const updatedSubCategory = await SubCategory.findByIdAndUpdate(id, updates, { new: true });

        res.status(200).json({ message: 'Sub-category updated successfully', data: updatedSubCategory });
    } catch (error) {
    ...
    try {
        ...
        if (associatedProducts.length > 0) {
            return res.status(400).json({
                message: 'Cannot delete sub-category. Products are associated with this sub-category.'
            });
        }
